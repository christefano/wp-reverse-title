...
The separator is applied via `document_title_separator`, which WordPress calls before `document_title_parts` inside `wp_get_document_title()`. If another plugin or theme hooks into `document_title_separator` with a priority higher than the default (10), it may override the custom separator. Try deactivating other plugins one at a time, and if you find the conflicting plugin let me know by opening a GitHub issue.
